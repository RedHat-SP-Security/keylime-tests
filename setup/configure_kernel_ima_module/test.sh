...
rlJournalStart
  ...
  if [ ! -e $COOKIE ]; then
    rlPhaseStartSetup "pre-reboot phase"
        ...
        rlRun "grubby --update-kernel DEFAULT --args 'ima_appraise=${IMA_APPRAISE} ima_canonical_fmt ima_policy=${IMA_POLICY} ima_template=${IMA_TEMPLATE} rd.shell rd.debug log_buf_len=1M' --remove-args='quiet'"
