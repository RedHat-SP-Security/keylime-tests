...
rlJournalStart
    ...
    rlPhaseStartSetup "Do the keylime setup for one-shot attestation"
        ...
        rlRun "cp ${ONESHOT_SCRIPT} ${ONESHOT_SCRIPT}.backup"
...
EOF"
    ...
    rlPhaseStartTest "Valid attestation with runtime and TPM policy"
        ...
        # Test with both runtime policy (IMA allowlist) and TPM policy (PCR values)
    ...
    rlPhaseStartTest "Policy failure - file not in allowlist"
        ...
        rlAssertGrep "Verifier HTTP Response Code: 200" $rlRun_LOG
        rlAssertGrep "'valid': False" $rlRun_LOG -i
    ...
    rlPhaseStartTest "Error handling - invalid verifier endpoint"
        ...
        rlAssertGrep "Failed to resolve 'nonexistent.invalid" $rlRun_LOG
    rlPhaseEnd
    ...
    rlPhaseStartCleanup "Do the keylime cleanup"
        ...
        rlRun "sed -i '/127.0.0.1 server/d' /etc/hosts"
    rlPhaseEnd
