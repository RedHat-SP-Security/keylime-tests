...
rlJournalStart
    ...
    rlPhaseStartTest "Add agent with measured boot policy and incorrect PCR banks"
        if [ "${AGENT_SERVICE}" == "PushAgent" ]; then
            rlRun -s "keylime_tenant -u $AGENT_ID --tpm_policy '{}' --runtime-policy policy.json -f /etc/hostname -c add --mb-policy mb_refstate3.txt ${TENANT_ARGS}"
        else
            rlRun -s "keylime_tenant -u $AGENT_ID --verify --tpm_policy '{}' --runtime-policy policy.json -f /etc/hostname -c add --mb-policy mb_refstate3.txt ${TENANT_ARGS}" 1
        fi
    ...
    rlPhaseStartTest "Try adding a mbpolicy with an existing name"
        rlRun -s "keylime_tenant -c addmbpolicy --mb-policy-name mypolicy --mb-policy mb_refstate3.txt" 1
    ...
    rlPhaseStartTest "Test updatembpolicy"
        rlRun -s "keylime_tenant -c updatembpolicy --mb-policy-name mypolicy --mb-policy mb_refstate3.txt"
        rlAssertGrep "{'code': 201, 'status': 'Created', 'results': {}}" "$rlRun_LOG"
        rlRun -s "keylime_tenant -c showmbpolicy --mb-policy-name mypolicy"
