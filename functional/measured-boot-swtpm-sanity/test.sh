...
rlJournalStart
    ...
    for mb_policy in "${MB_POLICIES[@]}"; do
        ...
        rlPhaseStartTest "Add agent with tpm_policy generated by different tools and correct PCR banks"
            rlRun -s "limeCtl agent add $AGENT_ID --tpm-policy '{}' --runtime-policy policy.json --mb-policy $mb_policy ${TENANT_ARGS}"
            rlRun "limeWaitForAgentStatus --field attestation_status $AGENT_ID 'PASS'"
        rlPhaseEnd
    done
    ...
    rlPhaseStartTest "Test listmbpolicy"
        rlRun -s "limeCtl measured-boot list"
    ...
    rlPhaseStartTest "Add an agent with an existing named mbpolicy."
        rlRun -s "limeCtl measured-boot push mypolicy --file mb_refstate.txt"
        rlRun -s "limeCtl agent add $AGENT_ID --mb-policy-name mypolicy ${TENANT_ARGS}"
    rlPhaseEnd
    ...
    rlPhaseStartTest "Add an agent with a new named mbpolicy." 
        rlRun "limeCtl agent remove $AGENT_ID"
        sleep 5
        rlRun -s "limeCtl agent add $AGENT_ID --mb-policy-name mypolicy2 --mb-policy mb_refstate.txt ${TENANT_ARGS}"
        rlRun -s "limeCtl measured-boot show mypolicy2"
    rlPhaseEnd
