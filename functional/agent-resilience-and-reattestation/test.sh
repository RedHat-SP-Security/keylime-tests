...
rlJournalStart
    ...
    rlPhaseStartTest "Stop agent so it stops sending quotes - should fail attestation"
        ...
        if [ "${AGENT_SERVICE}" == "PushAgent" ]; then
            rlRun "limeTIMEOUT=60 limeWaitForAgentStatus --field attestation_status '$AGENT_ID' 'FAIL'" 0 "Agent should fail attestation"
        else
            rlRun "limeTIMEOUT=60 limeWaitForAgentStatus '$AGENT_ID' '(Failed|Invalid Quote)'" 0 "Agent should fail attestation"
        fi
        rlRun -s "keylime_tenant -c cvlist"
    rlPhaseEnd
