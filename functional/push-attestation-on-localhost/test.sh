...
_EOF"
        ...
        rlAssertGrep "PATCH Response code:.*202 Accepted" "$(limePushAgentLogfile)" -E
        rlAssertGrep "Attestation [0-9]+ for agent '${AGENT_ID}' verified successfully" "$(limeVerifierLogfile)" -E

        # Store the index of the first attestation
        INDEX=$(grep -oE "Attestation [0-9]+ for agent '${AGENT_ID}' verified successfully" "$(limeVerifierLogfile)" |
            grep -o "Attestation [0-9]+" | grep "[0-9]+")
    rlPhaseEnd
    ...
    rlPhaseStartTest "Running allowed scripts should not affect attestation"
        ...
        rlAssertGrep "PATCH Response code:.*202 Accepted" "$(limePushAgentLogfile)" -E
        rlRun "rlWaitForCmd 'grep -qE \"Attestation $((INDEX + 1)) for agent .${AGENT_ID}. verified successfully\" \$(limeVerifierLogfile)' -m 120 -d 1"
    rlPhaseEnd
    ...
    rlPhaseStartTest "Fail keylime agent"
        ...
        rlRun "rlWaitForCmd 'grep -qE \"Attestation $((INDEX + 2)) for agent .${AGENT_ID}. failed verification\" \$(limeVerifierLogfile)' -m 120 -d 1"
