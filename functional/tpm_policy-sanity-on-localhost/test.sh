...
rlJournalStart
    ...
    rlPhaseStartTest "Add keylime agent"
        ...
        rlRun "keylime_tenant -v 127.0.0.1 -t 127.0.0.1 -u $AGENT_ID --tpm_policy ${TPM_POLICY} --allowlist policy.json -f /etc/hostname -c add"
    ...
    rlPhaseStartTest "Run keylime_tenant -c update, providing updated tpm_policy"
        ...
        rlRun "keylime_tenant -v 127.0.0.1 -t 127.0.0.1 -u $AGENT_ID --allowlist policy.json -f /etc/hostname --tpm_policy '${TPM_POLICY}' -c update"
        rlRun "limeWaitForAgentStatus $AGENT_ID 'Get Quote'"
        rlRun -s "keylime_tenant -c cvlist"
        rlAssertGrep "{'code': 200, 'status': 'Success', 'results': {'uuids':.*'$AGENT_ID'" $rlRun_LOG -E
    rlPhaseEnd
