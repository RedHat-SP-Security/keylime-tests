...
rlJournalStart
    ...
    rlPhaseStartTest "Try adding agent with PRC15 configured in tpm_policy"
        ...
        rlRun -s "limeCtl agent add $AGENT_ID --tpm-policy '${TPM_POLICY}' --runtime-policy policy.json --mb-refstate mb_refstate.txt" 1
    ...
    rlPhaseStartTest "Add agent with empty tpm_policy"
        rlRun -s "limeCtl agent add $AGENT_ID --tpm-policy '{}' --runtime-policy policy.json --mb-refstate mb_refstate.txt"
    ...
    rlPhaseStartTest "Create measured boot policy"
        ...
        rlRun "limePolicy generate measured-boot --eventlog-file /sys/kernel/security/tpm0/binary_bios_measurements $NO_SB_PARAM --output mb_refstate3.txt"
    ...
    for mb_policy in "${MB_POLICIES[@]}"; do
        rlPhaseStartTest "Add agent with tpm_policy generated by create_mb_refstate script"
            rlRun -s "limeCtl agent add $AGENT_ID --tpm-policy '{}' --runtime-policy policy.json --mb-refstate $mb_policy"
            rlRun "limeWaitForAgentStatus --field attestation_status $AGENT_ID 'PASS'"
            rlRun -s "limeCtl agent list"
            rlRun "limeAssertJsonField $rlRun_LOG uuids=$AGENT_ID"
        rlPhaseEnd
    ...
    rlPhaseStartTest "Add agent with incorrect tpm_policy"
        ...
        rlRun -s "limeCtl agent add $AGENT_ID --tpm-policy '{}' --runtime-policy policy.json --mb-refstate modified_mb_refstate.txt" 1
