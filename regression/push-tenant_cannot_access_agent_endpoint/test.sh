...
rlJournalStart
    ...
    rlPhaseStartTest "Tenant CAN access admin endpoints"
        # Test that tenant can list agents (admin endpoint)
        rlRun -s "keylime_tenant -c cvlist"
        rlAssertGrep "'code': 200" $rlRun_LOG
        rlAssertGrep "'status': 'Success'" $rlRun_LOG
        # Add agent to verifier (admin operation) with push model flag
        ...
        rlLog "Verified: Tenant can access admin endpoints"
    rlPhaseEnd
