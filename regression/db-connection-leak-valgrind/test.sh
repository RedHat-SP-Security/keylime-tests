...
analyze_valgrind_logs() {
    ...
    local log_files
    log_files=$(find "$log_dir" -name "verifier-*.log" -type f)
    ...
    for log_file in $log_files; do
        local pid
        pid=$(basename "$log_file" | sed 's/verifier-\(.*\)\.log/\1/')
        ...
        local open_fds
        open_fds=$(grep -c "^==${pid}== Open file descriptor" "$log_file" || true)
        ...
        if [ $leaked -gt 0 ]; then
            ...
            local sqlite_fds
            sqlite_fds=$(grep -c "^==${pid}== Open file descriptor.*cv_data.sqlite" "$log_file" || true)
...
rlJournalStart
    ...
    rlPhaseStartSetup "Setup keylime with valgrind"
        ...
        # Valgrind slows things down, so let's increase the timeout.
        export limeTIMEOUT=40
...
EOF
    ...
    rlPhaseStartCleanup "Cleanup"
        ...
        rlRun "rlBundleLogs valgrind_logs $VALGRIND_BASE_DIR/*"

        rlLogInfo "Phases analyzed:"
        find "$VALGRIND_BASE_DIR" -mindepth 1 -maxdepth 1 -type d | while read -r phase_dir; do
