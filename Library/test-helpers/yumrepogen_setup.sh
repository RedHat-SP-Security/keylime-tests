...
./yumrepogen -insecure -arch=$ARCH -compose-id=$(curl -kLs 'http://storage.tft.osci.redhat.com/composes-production.json' | grep -E -o "RHEL-$( rpm -q --qf '%{VERSION}' redhat-release )[^\"]+[0-9]" | sort | tail -1) -outfile /etc/yum.repos.d/yumrepogen.repo
