...
./yumrepogen -insecure -arch=x86_64 -compose-id=$(curl -kLs 'http://storage.tft.osci.redhat.com/composes-production.json' | grep -E -o "RHEL-9.6.0-[^\"]+" | head -1) -outfile /etc/yum.repos.d/yumrepogen.repo
