...
export __INTERNAL_limeLogPush_model_agent
[ -n "$__INTERNAL_limeLogPush_model_agent" ] || __INTERNAL_limeLogPush_model_agent="$__INTERNAL_limeTmpDir/push_agent.log"
...
export __INTERNAL_limeIMAKeysDir="${__INTERNAL_limeTmpDir}/ima_keys"
export limeIMACAPrivateKey=${__INTERNAL_limeIMAKeysDir}/imacakey.pem
export limeIMACACertificate=${__INTERNAL_limeIMAKeysDir}/imacacert.pem
export limeIMACACertificateDER=${__INTERNAL_limeIMAKeysDir}/imacacert.der
...
Generate and install IMA/EVM keys to /var/tmp/limeLib/ima_keys/ or to the specified destination.
...
limeInstallIMAKeys() {
    ...
    if ! [ -f ${IMAPrivateKey} ]; then
        local CONFIG=$( mktemp )
        # if IMA CA key is not present, generate CA key and certs as well
        if ! [ -f ${limeIMACAPrivateKey} ]; then
            cat <<END >${CONFIG}
[ req ]
...
[ issuer_dn ]
O = keylime-tests-IMA-CA
CN = IMA/EVM certificate signing key
emailAddress = lime@keylime-tests
...
END
            ...
            openssl req -new -x509 -nodes -out ${limeIMACACertificate} -sha256 -pkeyopt rsa_keygen_bits:3072 -days 365 -batch -config $CONFIG -keyout ${limeIMACAPrivateKey} && \
            openssl x509 -in ${limeIMACACertificate} -out ${limeIMACACertificateDER} -outform der
            if [ $? -ne 0 ]; then
                echo "Error: Failed to generate IMA CA key and certificates"
                return 1
            fi
        fi
        # now proceed with IMA key itself
        cat <<END >${CONFIG}
[ req ]
default_md = sha256
...
END
        openssl genrsa -out ${IMAPrivateKey} 3072 && \
        openssl req -new -key ${IMAPrivateKey} -out imacsrrsa.pem -batch -config ${CONFIG} && \
        openssl x509 -req -in imacsrrsa.pem -CA ${limeIMACACertificate} -CAkey ${limeIMACAPrivateKey} -outform der -out ${IMACertificateDER} -days 90 -extensions ext -extfile ${CONFIG} && \
        openssl rsa -pubout -in ${IMAPrivateKey} -out ${IMAPublicKey}
        if [ $? -ne 0 ]; then
            echo "Error: Failed to generate IMA key and certificates"
            return 2
        fi
...
Prints to STDOUT filepath to a log file containing push attestation Agent logs
...
# Wait for push agent to complete attestation
# Arguments:
#   $1 - agent_id
#   $2 - timeout in seconds (optional, defaults to 30)
limePushWaitForAttestation() {
    local agent_id="$1"
    local timeout="${2:-30}"
    local count=0

    rlLog "Waiting for push attestation completion for agent: $agent_id"

    while [ "$count" -lt "$timeout" ]; do
        # Check agent logs for completion/success indicators
        if [ -f /tmp/push-agent.log ]; then
            if grep -q "attestation.*complete\|attestation.*success\|State.*Complete" /tmp/push-agent.log 2>/dev/null; then
                rlLog "Push attestation completed successfully"
                return 0
            fi

            # Check for failures
            if grep -q "failed\|error\|ERROR\|State.*Failed" /tmp/push-agent.log 2>/dev/null; then
                rlLog "Push attestation failed - check logs"
                limePushSubmitLogs
                return 1
            fi
        fi
    ...
    rlLog "Timeout waiting for push attestation"
    limePushSubmitLogs
    return 1
}
