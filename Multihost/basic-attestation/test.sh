...
set timeout 20
spawn keylime_tenant -v ${VERIFIER_IP} -t ${AGENT2_IP} -u ${AGENT2_ID} --allowlist policy2.json --include payload-${REVOCATION_SCRIPT_TYPE} --cert default -c add
...
_EOF"
    ...
    rlPhaseStartTest "keylime attestation test: Add Agent"
        ...
        rlRun "cat > script.expect <<_EOF
set timeout 20
spawn keylime_tenant -v ${VERIFIER_IP} -t ${AGENT_IP} -u ${AGENT_ID} --allowlist policy.json --include payload-${REVOCATION_SCRIPT_TYPE} --cert default -c add
expect \"Please enter the password to decrypt your keystore:\"
send \"keylime\n\"
expect eof
_EOF"
